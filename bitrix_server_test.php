<?php
#####################################
define('LAST_MODIFIED','11.03.2026');
#####################################
...
elseif (!empty($_GET['session_test']))
{
	session_start();
...
elseif (!empty($_GET['memory_test']))
{
	...
	for ($i = 1; $i <= $max * 8; $i++)
	{
		$a[] = str_repeat('a', 1024 * 128); // 128 KB
	}
	die("SUCCESS");
}
elseif (isset($_GET['killme']) && $_GET['killme'] == 'Y')
{
	unlink(__FILE__);
